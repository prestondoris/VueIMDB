    ...
    <v-layout wrap>
      <v-flex xs12 mr-1 ml-1>
        <v-card>
          ...
          <v-card-actions>
            <v-btn flat color="green" @click="goBack()">Return</v-btn>
          </v-card-actions>
        </v-card>
      </v-flex>
    </v-layout>
...
<script>
    ...
    export default {
        ...
        methods: {
            async getMovie() {
                const response = await MovieService.getMovie('?apikey=9325751e&Content-Type=application/json&i='+this.id)
                this.loading = false
                this.singleMovie = response.data
                this.ratings = this.singleMovie.Ratings
                this.ratings.forEach(function(el){
                    el.Value = parseFloat(el.Value.split(/\/|%/)[0])
                    el.Value = el.Value <= 10 ? el.Value / 2 : el.Value / 20
                })
            },
            goBack () {
